...
class Game:
	...
	def init_game(self):
		self.player = Player_pix(all_sprites, load_image('images\\hero_sheet.png'), 6, 2, 300, 400)
	...
	def generate_level1(self):
		...
		for y in range(len(level)):
			for x in range(len(level[y])):
				if level[y][x] == '.':
					Tile(load_image('tiles\\road.png'), tile_width * x, tile_height * y, all_sprites)
				elif level[y][x] == '#':
					Object(load_image('tiles\\wall2.png'), x * tile_width, y * tile_height, (60, 60), all_sprites)
		decorations = load_level('decorations_map.map')
		for y in range(len(decorations)):
			for x in range(len(decorations[y])):
				if decorations[y][x] == '.':
					Tile(load_image('images\\grass.png'), tile_width * x, tile_height * y, all_sprites)
				elif decorations[y][x] == '@':
					Object(load_image('decorations\\home1.png'), x * tile_width, y * tile_height, (120, 200), all_sprites)
				elif decorations[y][x] == '$':
					Object(load_image('decorations\\tavern.png'), x * tile_width, y * tile_height, (120, 200), all_sprites)
				elif decorations[y][x] == '!':
					Tile(load_image('images\\grass.png'), tile_width * x, tile_height * y, all_sprites)
					Object(load_image('decorations\\tree.png'), x * tile_width, y * tile_height,
	...
	def generate_level3(self):
		...
		for y in range(len(level)):
			for x in range(len(level[y])):
				if level[y][x] == '!':
					Object(load_image('decorations\\tree.png'), x * tile_width + 3800, y * tile_height,
					       (tile_width, 20 + tile_height), all_sprites)
				elif level[y][x] == '@':
					Enemy(load_image('images\\demon_ic.png'), x * tile_width + 3800, y * tile_height,
					       (tile_width, tile_height), all_sprites)
		
	def run(self):
		self.fon = pygame.transform.scale(load_image('images\\fon.png'), self.screen_size)
		while self.running:
			...
			if self.player.pos[0] >= 3000 and self.quest_started:
				pygame.mixer.music.load('../data/music/Tooth-and-Claw.wav')
				pygame.mixer.music.play(-1)
				pygame.mixer.music.set_volume(0.2)
				self.fon = pygame.transform.scale(load_image('images\\dark_fon.png'), self.screen_size)
